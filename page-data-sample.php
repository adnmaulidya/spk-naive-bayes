...
<?php } else if($view == "save"){
	...
	if(!empty($d->nama)){ $d->nama = strtoupper(mysqli_escape_string($con,$d->nama));
		$query = mysqli_query($con,"select * from mila_sample where NIM = '$d->nim'");
		if(mysqli_num_rows($query) == 0){
			$q = mysqli_query($con,"insert into mila_sample (ID, NIM, NAMA, PRODI, JURUSAN, IPK,
				JENKEL, JMLPIAGAM) value ($id, $d->nim, '$d->nama', $d->prodi,
				$d->jurusan, $d->ipk, $d->semester, '$d->jenkel', $d->jmlpiagam, $d->beasiswa);");
			if($q) echo 1;
			else echo 4;
		}else{
			echo 2;
	...
	if(!empty($d->nama)){ $d->nama = strtoupper(mysqli_escape_string($con,$d->nama));
		...
		JURUSAN = $d->jurusan, IPK = $d->ipk, SEMESTER = $d->semester, JENKEL = '$d->jenkel',
		JMLPIAGAM = $d->jmlpiagam where ID = $d->id;");
		if($q) echo 1;
		else echo 2;
	} else echo 3;
			...
			<?php
				...
				if($query && mysqli_num_rows($query) > 0){ $num=0;
					while($row = mysqli_fetch_object($query)){$num++;
						$qry = mysqli_query($con,"select STATUS from mila_mawapres where ID=$row->STATUS;");
						...
						<tr id="tr<?php echo $row->ID;?>">
							...
							<td><?php echo (isset($bsw->STATUS))?$bsw->STATUS:"TIDAK";?></td>
							<td>
